<template>
  ...
  >
    ...
    <div class="px-4 py-3 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-white dark:bg-slate-900 shrink-0 z-30 relative shadow-sm h-16">
      <div class="flex items-center gap-3">
        ...
        <div v-if="focusedComponent" class="flex items-center gap-2 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 px-2 py-0.5 rounded text-xs font-medium border border-indigo-100 dark:border-indigo-800 animate-in fade-in slide-in-from-left-2">
          <span>Focus: {{ focusedComponent.replace(/_/g, ' ') }}</span>
          <button @click="clearFocus" class="hover:text-indigo-800 dark:hover:text-indigo-200"><svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
        </div>
      ...
      <div class="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg border border-slate-200 dark:border-slate-700">
        <button @click.stop="zoomOut" :disabled="zoomIndex === 0" class="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded-md disabled:opacity-30 disabled:cursor-not-allowed transition-all text-slate-600 dark:text-slate-400">
        ...
        <span class="text-[10px] font-bold font-mono w-10 text-center text-slate-600 dark:text-slate-300 select-none">{{ Math.round(currentZoom * 100) }}%</span>
        <button @click.stop="zoomIn" :disabled="zoomIndex === zoomLevels.length - 1" class="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded-md disabled:opacity-30 disabled:cursor-not-allowed transition-all text-slate-600 dark:text-slate-400">
          <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
        </button>
        <div class="w-px h-4 bg-slate-300 dark:bg-slate-600 mx-1"></div>
        <button @click.stop="resetZoom" class="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded-md transition-all text-slate-600 dark:text-slate-400">
      ...
      <div class="relative min-w-full h-full" :style="{ width: `${currentZoom * 100}%` }">
        <div class="absolute inset-0 pointer-events-none z-0">
          ...
          >
            ...
            >
              <span class="text-xs text-slate-600 dark:text-slate-300">{{ formatTickDate(tick) }}</span>
              <span class="text-[10px] text-slate-500 dark:text-slate-500">{{ formatTickTime(tick) }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="pt-20 pb-8 space-y-4 z-10 relative min-h-[250px]">
          <div
              ...
              :class="focusedComponent && focusedComponent !== comp ? 'opacity-20 blur-[1px]' : 'hover:bg-black/5 dark:hover:bg-white/5'"
          >
            <div
                ...
                :class="focusedComponent === comp ? 'bg-indigo-50/95 dark:bg-indigo-900/40 border-r-indigo-200' : 'bg-slate-50/95 dark:bg-slate-900/95'"
            >
              <div class="text-right leading-tight">
                <div class="text-sm font-bold truncate capitalize" :class="focusedComponent === comp ? 'text-indigo-700 dark:text-indigo-300' : 'text-slate-700 dark:text-slate-300'">
                  {{ comp.replace(/_/g, ' ') }}
                </div>
                <div class="text-[10px] font-normal mt-0.5" :class="focusedComponent === comp ? 'text-indigo-400' : 'text-slate-400'">
                  {{ focusedComponent === comp ? 'Focused' : 'Double-click' }}
                </div>
              </div>
            </div>

            <div class="flex-1 h-12 mx-4 rounded-full bg-slate-200 dark:bg-slate-800/80 relative overflow-hidden ring-1 ring-slate-300 dark:ring-slate-700">
              <div
    ...
    >
      <div class="flex justify-between items-start mb-3">
        <div class="flex items-center gap-2">
          <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold uppercase tracking-wide"
                :class="selectedAlarm.severity === 'failed' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'">
            {{ selectedAlarm.severity }}
          </span>
          <span v-if="selectedAlarm.alarm_code" class="text-xs font-mono text-slate-500 dark:text-slate-400">
            #{{ selectedAlarm.alarm_code }}
          </span>
        </div>
        <button @click="closePopover" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
          <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
        </button>
      </div>

      <p class="text-sm font-medium text-slate-800 dark:text-slate-200 mb-3 leading-snug">{{ selectedAlarm.message }}</p>
      ...
      <div class="bg-slate-50 dark:bg-slate-900 rounded-lg p-3 space-y-2 text-xs">
        <div class="flex justify-between border-b border-slate-200 dark:border-slate-700 pb-2">
          ...
          <span class="font-mono text-slate-700 dark:text-slate-300">{{ new Date(selectedAlarm.start_time).toLocaleString() }}</span>
        </div>
        ...
        <div v-if="selectedAlarm.data?.value" class="flex justify-between pt-1">
          ...
          <span class="font-mono font-bold text-slate-900 dark:text-white">{{ selectedAlarm.data.value }}</span>
        </div>
      </div>
    </div>
...
const toggleFocus = (comp) => { focusedComponent.value = focusedComponent.value === comp ? null : comp }
const clearFocus = () => { focusedComponent.value = null }
const timelineMeta = computed(() => {
  const data = props.alarms
  if (!data.length) return { min: 0, max: 0, totalRange: 0, components: [] }
  const timestamps = data.flatMap(a => [new Date(a.start_time).getTime(), new Date(a.end_time).getTime()])
  const min = Math.min(...timestamps)
  const max = Math.max(...timestamps)
  const buffer = 7200000
  return { min: min - buffer, max: max + buffer, totalRange: (max + buffer) - (min - buffer), components: [...new Set(data.map(a => a.component))].sort() }
})
const visibleComponents = computed(() => focusedComponent.value ? [focusedComponent.value] : timelineMeta.value.components)

// --- Zoom Logic ---
const zoomLevels = [1, 1.5, 2, 4, 8, 16, 24, 32]
const zoomIndex = ref(0)
const currentZoom = computed(() => zoomLevels[zoomIndex.value])
const zoomIn = () => { if (zoomIndex.value < zoomLevels.length - 1) zoomIndex.value++ }
const zoomOut = () => { if (zoomIndex.value > 0) zoomIndex.value-- }
const resetZoom = () => { zoomIndex.value = 0 }
const handleWheel = (e) => {
...
const openPopover = (event, alarm) => {
  ...
  if (x + popoverWidth > containerRect.width) x = event.clientX - containerRect.left - popoverWidth - 20
  if (y + popoverHeight > containerRect.height) y = event.clientY - containerRect.top - popoverHeight - 10
  if (x < 10) x = 10
  if (y < 10) y = 10
...
const closePopover = () => { selectedAlarm.value = null }
const popoverStyle = computed(() => ({ top: `${popoverPosition.value.y}px`, left: `${popoverPosition.value.x}px` }))
...
// --- Helpers ---
const getTickPosition = (ts) => ((ts - timelineMeta.value.min) / timelineMeta.value.totalRange) * 100
...
const getSeverityClass = (s) => {
  switch(s) { case 'failed': return 'bg-red-500'; case 'critical': return 'bg-red-600'; case 'warning': return 'bg-amber-400'; default: return 'bg-blue-400' }
}
const formatTickDate = (ts) => new Date(ts).toLocaleDateString(undefined, { month:'short', day:'numeric'})
const formatTickTime = (ts) => new Date(ts).toLocaleTimeString([], { hour:'2-digit', minute:'2-digit'})
</script>
...
.dark .custom-scrollbar::-webkit-scrollbar-thumb { background: #334155; border-color: #020617; }
</style>