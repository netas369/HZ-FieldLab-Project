<template>
  <div class="mb-6">
        ...
        >
          ...
          >
            <div class="flex justify-between w-full items-center mb-1">
              ...
              <div v-if="activeEntryId === entry.id" class="w-2 h-2 rounded-full bg-indigo-500 animate-pulse"></div>
            </div>
    ...
    <Teleport to="body">
      <div v-if="showClearConfirm" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity" @click="showClearConfirm = false"></div>

        <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 max-w-sm w-full p-6 transform transition-all">
          ...
          <div class="flex gap-3">
            <button @click="showClearConfirm = false" class="flex-1 px-4 py-2 text-sm font-semibold text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-xl transition-colors">
              Cancel
            </button>
            <button @click="confirmClear" class="flex-1 px-4 py-2 text-sm font-semibold text-white bg-red-500 hover:bg-red-600 rounded-xl shadow-md transition-colors active:scale-95">
              Clear All
            </button>
          </div>
        </div>
