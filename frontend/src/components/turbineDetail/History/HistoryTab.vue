<template>
  <div
    class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 relative"
  >
    <div class="mb-6 border-b border-slate-200 dark:border-slate-700 pb-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
        ...
        >
          ...
          <span>{{
            historyStore.loading ? 'Processing...' : 'Fetch Data (~' + estimatedDuration + 's)'
          }}</span>
        </button>
      </div>
    </div>
    ...
    <HistoryRecords
      ...
      @clear-all="historyStore.clearAllForTurbine(turbineId)"
    />
    ...
    >
      ...
      <button
        class="px-4 py-2 bg-white dark:bg-slate-800 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 rounded-lg text-sm font-medium hover:bg-red-50 transition-colors"
...
const activeTurbinePayload = computed(
  () => historyStore.activeDataMap[props.turbineId]?.payload || null
)
...
const estimatedDuration = computed(() =>
  Math.max((dayCount.value * SECONDS_PER_DAY).toFixed(1), 0.5)
)
...
const telemetryStats = computed(() => {
  ...
  const avgPower = (
    fullScada.reduce((sum, d) => sum + parseFloat(d.power_kw), 0) / fullScada.length
  ).toFixed(0)
  const maxWind = Math.max(...fullScada.map((d) => parseFloat(d.wind_speed_ms))).toFixed(1)
  const avgTemp = (
    fullScada.reduce((sum, d) => sum + parseFloat(d.ambient_temp_c), 0) / fullScada.length
  ).toFixed(1)
...
const chartOptions = computed(() => ({
  ...
  scales: {
    ...
    y: {
      ...
      position: 'left',
      grid: {
        color: selectedMetric.value === 'alarms' ? 'transparent' : 'rgba(148, 163, 184, 0.1)',
      },
...
const chartData = computed(() => {
  ...
  const timeLabels = scadaList.value.map((d) =>
    new Date(d.reading_timestamp).toLocaleTimeString([], {
      month: 'short',
      day: 'numeric',
      hour: '2-digit',
      minute: '2-digit',
  ...
  const dataset = (label, data, color, yAxis = 'y', fill = false, type = 'line') => ({
    ...
    data,
    borderColor: color,
    backgroundColor: fill ? 'rgba(139, 92, 246, 0.1)' : undefined,
    fill,
    yAxisID: yAxis,
    pointRadius: 0,
    pointHoverRadius: 4,
  ...
  if (selectedMetric.value === 'performance')
    return {
      ...
      datasets: [
        dataset(
          'Power Output (kW)',
          scadaList.value.map((d) => d.power_kw),
          '#8b5cf6',
          'y',
          true
        ),
        dataset(
          'Wind Speed (m/s)',
          scadaList.value.map((d) => d.wind_speed_ms),
          '#3b82f6',
          'y1'
        ),
      ],
  ...
  if (selectedMetric.value === 'temperatures')
    return {
      ...
      datasets: [
        dataset(
          'Gearbox',
          tempList.value.map((d) => d.gearbox_bearing_temp_c),
          '#f97316'
        ),
        dataset(
          'Stator',
          tempList.value.map((d) => d.generator_stator_temp_c),
          '#ef4444'
        ),
        dataset(
          'Main Bearing',
          tempList.value.map((d) => d.main_bearing_temp_c),
          '#eab308'
        ),
      ],
    }
  if (selectedMetric.value === 'vibration_bearings')
    return {
      ...
      datasets: [
        dataset(
          'Main Bearing',
          vibList.value.map((d) => d.main_bearing_vibration_rms_mms),
          '#10b981'
        ),
        dataset(
          'Gearbox',
          vibList.value.map((d) => d.gearbox_vibration_axial_mms),
          '#6366f1'
        ),
        dataset(
          'Generator',
          vibList.value.map((d) => d.generator_vibration_de_mms),
          '#ec4899'
        ),
      ],
    }
  if (selectedMetric.value === 'hydraulics')
    return {
      ...
      datasets: [
        dataset(
          'Hydraulic Pressure',
          hydroList.value.map((d) => d.hydraulic_pressure_bar),
          '#06b6d4'
        ),
        dataset(
          'Gearbox Oil',
          hydroList.value.map((d) => d.gearbox_oil_pressure_bar),
          '#84cc16',
          'y1'
        ),
      ],
