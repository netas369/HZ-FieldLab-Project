<template>
  <div class="space-y-8 animate-fade-in">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <!-- Overall Score Card -->
      <div
        class="md:col-span-2 bg-gradient-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 p-6 flex items-center justify-between shadow-sm relative overflow-hidden"
      >
        <div
          class="absolute right-0 top-0 w-32 h-32 bg-current opacity-5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"
          :class="getHealthColor(overallHealth.overall_health_score)"
        />
        <div>
          <h3 class="text-lg font-semibold text-slate-700 dark:text-slate-300 mb-1">
            Overall Turbine Health
          </h3>
          <p class="text-slate-500 dark:text-slate-400 text-sm mb-4">
            Based on {{ analysisPeriod.days_analyzed || healthData.period_days }} days of sensor
            data
            <span
              v-if="analysisPeriod.confidence_level"
              class="ml-2 px-2 py-0.5 rounded text-xs font-medium"
              :class="getConfidenceBadge(analysisPeriod.confidence_level)"
            >
          ...
          <div class="flex items-center gap-3 flex-wrap">
            <span
              class="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide border bg-white dark:bg-slate-900"
              :class="getStatusBadgeColor(overallHealth.status)"
            >
          ...
          <!-- Recommendation -->
          <p
            v-if="overallHealth.recommendation"
            class="mt-3 text-xs text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-900/50 px-3 py-2 rounded-lg"
          >
        ...
        <div class="relative w-24 h-24 flex items-center justify-center flex-shrink-0">
          ...
          >
            ...
            <path
              :class="getHealthColor(overallHealth.overall_health_score)"
              :stroke-dasharray="`${overallHealth.overall_health_score || 0}, 100`"
              d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
              fill="none"
              stroke="currentColor"
              stroke-width="3"
          ...
          <div class="absolute flex flex-col items-center">
            <span class="text-2xl font-bold text-slate-900 dark:text-white">{{
              overallHealth.overall_health_score?.toFixed(0) ?? '-'
            }}</span>
      ...
      <!-- Analysis Info Card -->
      <div
        class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 flex flex-col justify-center"
      >
        <div
          class="text-sm text-slate-500 dark:text-slate-400 mb-2 font-medium uppercase tracking-wider"
        >
          Analysis Window
        </div>
        <div class="text-2xl font-bold text-slate-900 dark:text-white mb-1">
          {{ analysisPeriod.days_analyzed || healthData.period_days }} Days
        </div>
        <div class="text-xs text-slate-400 mb-3">
          Updated: {{ formatDate(healthData.calculation_timestamp) }}
        </div>
        <div
          v-if="analysisPeriod.data_availability"
          class="text-xs space-y-1 pt-3 border-t border-slate-100 dark:border-slate-700"
        >
          <div class="flex justify-between">
            <span class="text-slate-500">Data from:</span>
            <span class="text-slate-700 dark:text-slate-300">{{
              formatDate(analysisPeriod.data_availability.oldest_reading)
            }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-slate-500">Data to:</span>
            <span class="text-slate-700 dark:text-slate-300">{{
              formatDate(analysisPeriod.data_availability.newest_reading)
            }}</span>
          </div>
        </div>
      </div>
        ...
        >
          ...
          >
            <div class="flex items-center gap-3">
              <div
                class="relative w-10 h-10 flex items-center justify-center rounded-full bg-slate-50 dark:bg-slate-900 border"
                :class="getBorderColor(comp.health_score)"
              >
                <span
                  v-if="comp.health_score !== null"
                  class="font-bold text-sm"
                  :class="getTextColor(comp.health_score)"
                >
            ...
            <button class="text-slate-400 group-hover:text-indigo-500 transition-colors">
              <svg
                ...
                :class="expandedComponent === key ? 'rotate-180' : ''"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
          ...
          <!-- Expanded Content -->
          <div
            v-show="expandedComponent === key"
            class="border-t border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/30 p-4 text-sm space-y-4"
          >
            <!-- Deterioration Analysis -->
            <div
              v-if="comp.deterioration_analysis?.can_analyze"
              class="bg-white dark:bg-slate-800 rounded-lg p-3 border border-slate-200 dark:border-slate-700"
            >
              <div class="flex items-center justify-between mb-2">
                <h5 class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">
                  Deterioration Analysis
                </h5>
                <span
                  class="text-[10px] px-2 py-0.5 rounded-full font-medium"
                  :class="getConfidenceBadge(comp.deterioration_analysis.analysis_confidence)"
                >
              ...
              <div class="grid grid-cols-3 gap-2 text-center">
                <div class="p-2 bg-slate-50 dark:bg-slate-900/50 rounded">
                  ...
                  <div
                    ...
                    :class="
                      getPredictionColor(comp.deterioration_analysis.predictions?.days_to_warning)
                    "
                  >
                ...
                <div class="p-2 bg-slate-50 dark:bg-slate-900/50 rounded">
                  ...
                  <div
                    class="font-mono font-bold text-sm"
                    :class="
                      getPredictionColor(comp.deterioration_analysis.predictions?.days_to_critical)
                    "
                  >
                ...
                <div class="p-2 bg-slate-50 dark:bg-slate-900/50 rounded">
                  ...
                  <div
                    class="font-mono font-bold text-sm"
                    :class="
                      getPredictionColor(comp.deterioration_analysis.predictions?.days_to_failed)
                    "
                  >
              ...
              <!-- Recommendation -->
              <p
                v-if="comp.deterioration_analysis.recommendation"
                class="mt-2 text-xs text-slate-600 dark:text-slate-400 italic"
              >
            ...
            <div v-if="comp.sensor_trends && Object.keys(comp.sensor_trends).length > 0">
              <h5 class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase mb-2">
                Sensor Trends
              </h5>
              <div class="space-y-2">
                <div
                ...
                >
                  <div class="flex items-center justify-between mb-1">
                    ...
                    <div class="flex items-center gap-2">
                      <span
                        v-if="trend.current_zone"
                        class="text-[10px] px-1.5 py-0.5 rounded font-medium"
                        :class="getZoneBadge(trend.current_zone)"
                      >
                        {{ trend.current_zone }}
                      </span>
                      <span
                        v-if="trend.trend?.direction"
                        class="text-[10px] flex items-center gap-0.5"
                        :class="getTrendDirectionColor(trend.trend.direction)"
                      >
                        <svg
                          v-if="trend.trend.direction === 'INCREASING'"
                          class="w-3 h-3"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        ...
                        <svg
                          v-else-if="trend.trend.direction === 'DECREASING'"
                          class="w-3 h-3"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                  ...
                  >
                    ...
                    <div>
                      ...
                      <span
                        ...
                        :class="
                          trend.trend?.r_squared >= 0.7
                            ? 'text-green-600'
                            : trend.trend?.r_squared >= 0.4
                              ? 'text-amber-600'
                              : 'text-slate-400'
                        "
                      >
                    ...
                    <div>
                      ...
                      <span
                        class="ml-1 font-mono"
                        :class="trend.trend?.slope_per_year > 0 ? 'text-red-500' : 'text-green-500'"
                      >
                        {{ trend.trend?.slope_per_year > 0 ? '+' : ''
                        }}{{ trend.trend?.slope_per_year?.toFixed(2) }}
                      </span>
                    </div>
                  </div>
                  <div
            ...
            >
              ...
              >
                <div class="text-[10px] text-slate-400 uppercase">
                  {{ healthData.period_days || 365 }} Days Ago
                </div>
              ...
              >
                ...
                <div
                  ...
                  :class="
                    comp.trend_analysis.deterioration_level === 'STABLE'
                      ? 'text-green-500'
                      : 'text-red-500'
                  "
                >
            ...
            <div v-if="comp.current_data">
              ...
              >
                <div class="grid grid-cols-2 gap-x-4 gap-y-1">
                  ...
                  >
                    <span class="text-slate-600 dark:text-slate-400 capitalize truncate pr-2">{{
                      formatSensorName(dKey)
                    }}</span>
                ...
                <div
                  v-if="comp.current_data.timestamp"
                  class="mt-2 pt-2 border-t border-slate-100 dark:border-slate-700 text-[10px] text-slate-400"
                >
...
const analysisPeriod = computed(
  () => props.healthData?.analysis_period ?? { days_analyzed: props.healthData?.period_days }
)
...
const getStatusBadgeColor = (status) => {
  switch (status) {
    ...
    case 'FAIR':
      return 'bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-900'
    case 'POOR':
