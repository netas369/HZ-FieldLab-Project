    ...
    <div :class="['rounded-xl p-4 border-l-4', getOverallStatusColor()]">
      <div class="flex items-center gap-3">
        <div class="flex-shrink-0">
          <svg
            v-if="temperature.overall_temperature_status?.color === 'green'"
        ...
        <div class="flex-1">
          ...
          <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Last reading: {{ formatDateTime(temperature.latest_reading) }} • Load Factor:
            {{ formatNumber(temperature.load_factor * 100, 0) }}%
          </p>
        </div>
      </div>
    </div>
