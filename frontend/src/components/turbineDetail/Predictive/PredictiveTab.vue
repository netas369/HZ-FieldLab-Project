  ...
  <div class="space-y-6 animate-fade-in">
    ...
    <div class="flex items-center justify-between flex-wrap gap-4">
      ...
      <div class="flex items-center gap-3">
        <span v-if="analysisConfidence" class="text-xs px-2 py-1 rounded-full font-medium" :class="getConfidenceBadge(analysisConfidence)">
          {{ analysisConfidence }} Confidence
        </span>
        <span class="text-xs font-mono text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div v-if="fastestDeteriorating" class="bg-gradient-to-br from-red-50 to-white dark:from-red-900/20 dark:to-slate-800 border border-red-100 dark:border-red-900/30 rounded-xl p-6 relative overflow-hidden shadow-sm">
        ...
        <div class="relative z-10">
          ...
          <div class="text-sm text-slate-500 dark:text-slate-400 mb-4">Fastest deteriorating component</div>
          ...
          <div class="space-y-3">
            ...
            <div class="flex items-center justify-between p-3 bg-white/60 dark:bg-slate-900/50 rounded-lg border border-red-100 dark:border-red-900/20">
              ...
              <span class="font-mono font-bold text-lg" :class="getHealthTextColor(fastestDeteriorating.health_score)">
            ...
            <div class="flex items-center justify-between p-3 bg-white/60 dark:bg-slate-900/50 rounded-lg border border-red-100 dark:border-red-900/20">
              ...
              <span class="px-2 py-0.5 rounded text-xs font-bold uppercase" :class="getDeteriorationBadge(fastestDeteriorating.deterioration_level)">
            ...
            <div class="flex items-center justify-between p-3 bg-white/60 dark:bg-slate-900/50 rounded-lg border border-red-100 dark:border-red-900/20">
              ...
              <span class="font-mono font-bold text-slate-900 dark:text-white flex items-center gap-1">
                <svg v-if="fastestDeteriorating.days_to_critical && fastestDeteriorating.days_to_critical < 90" class="w-4 h-4 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                </svg>
                {{ fastestDeteriorating.days_to_critical ?? '—' }}
                <span v-if="fastestDeteriorating.days_to_critical" class="text-slate-400 text-sm font-normal">days</span>
              </span>
            </div>

            <!-- Analysis Confidence -->
            <div v-if="fastestDeteriorating.analysis_confidence" class="flex items-center justify-between p-3 bg-white/60 dark:bg-slate-900/50 rounded-lg border border-red-100 dark:border-red-900/20">
              <span class="text-sm text-slate-600 dark:text-slate-400">Analysis Confidence</span>
              <span class="px-2 py-0.5 rounded text-xs font-medium" :class="getConfidenceBadge(fastestDeteriorating.analysis_confidence)">
          ...
          <!-- Recommendation -->
          <div v-if="fastestDeteriorating.recommendation" class="mt-4 p-3 bg-red-100/50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
      ...
      <div v-else class="bg-gradient-to-br from-green-50 to-white dark:from-green-900/20 dark:to-slate-800 border border-green-100 dark:border-green-900/30 rounded-xl p-6 relative overflow-hidden shadow-sm">
        <div class="absolute -right-6 -top-6 w-32 h-32 bg-green-500/10 rounded-full blur-2xl pointer-events-none"></div>
        <div class="relative z-10 text-center py-6">
          ...
          <div class="text-xl font-bold text-slate-900 dark:text-white mb-2">All Systems Healthy</div>
          <p class="text-sm text-slate-500 dark:text-slate-400">No components require immediate attention</p>
        </div>
      </div>
      ...
      <div class="lg:col-span-2 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm flex flex-col">
        ...
        <div class="overflow-x-auto flex-1">
          <table class="w-full text-sm text-left">
            <thead class="bg-slate-50 dark:bg-slate-900/50 text-xs text-slate-500 uppercase font-semibold sticky top-0">
            <tr>
            ...
            <tr
                ...
                :class="{ 'bg-red-50/50 dark:bg-red-900/10': trend.days_to_critical && trend.days_to_critical < 60 }"
            >
              ...
              <td class="px-6 py-3 text-right">
                  <span class="font-mono font-medium" :class="getHealthTextColor(trend.health_score)">
                    {{ trend.health_score?.toFixed(1) ?? '-' }}
                  </span>
              </td>
              <td class="px-6 py-3 text-center">
                  <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase" :class="getStatusBadge(trend.status)">
                    {{ trend.status }}
                  </span>
              </td>
              <td class="px-6 py-3 text-center">
              ...
              <td class="px-6 py-3 text-center">
                  <span v-if="trend.analysis_confidence" class="px-2 py-0.5 rounded text-[10px] font-medium" :class="getConfidenceBadge(trend.analysis_confidence)">
              ...
              <td class="px-6 py-3 text-right font-mono">
                <div class="flex items-center justify-end gap-2">
                  ...
                  <svg v-if="trend.days_to_critical && trend.days_to_critical < 60" class="w-4 h-4 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                  </svg>
                </div>
              </td>
    ...
    <!-- Legend -->
    <div class="flex flex-wrap gap-4 text-xs text-slate-500 dark:text-slate-400 pt-4 border-t border-slate-200 dark:border-slate-700">
      <div class="flex items-center gap-2">
...
const analysisConfidence = computed(() => {
  ...
  const confidences = sortedTrends.value
      .map(t => t.analysis_confidence)
      .filter(Boolean)
...
const getConfidenceBadge = (confidence) => {
  switch (confidence) {
    ...
    case 'VERY_LOW': return 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400'
    default: return 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400'
  }
}
