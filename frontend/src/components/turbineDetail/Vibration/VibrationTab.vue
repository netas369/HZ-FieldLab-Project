<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <span class="text-xs font-medium px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-800 dark:bg-slate-700 dark:text-slate-300">
        Live Data
      </span>
    </div>
      ...
      >
        ...
        <div class="grid grid-cols-2 gap-2">
          <div>
            ...
            <p class="text-xl font-bold text-slate-900 dark:text-white">{{ formatNumber(turbine.gearbox_vibration_axial, 2) }}</p>
          </div>
          <div>
            <p class="text-xs text-slate-500 mb-1">Radial</p>
            <p class="text-xl font-bold text-slate-900 dark:text-white">{{ formatNumber(turbine.gearbox_vibration_radial, 2) }}</p>
          </div>
        </div>
      </MetricCard>
      ...
      >
        ...
        <div class="space-y-2">
          <div class="flex justify-between items-center">
            ...
            <span class="font-bold text-slate-900 dark:text-white">{{ formatNumber(turbine.generator_vibration_de, 2) }}</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-sm text-slate-500">Non-Drive End</span>
            <span class="font-bold text-slate-900 dark:text-white">{{ formatNumber(turbine.generator_vibration_nde, 2) }}</span>
          </div>
        </div>
      </MetricCard>
      ...
      >
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="bg-slate-50 dark:bg-slate-900 p-2 rounded-lg text-center">
            <p class="text-lg font-bold text-slate-900 dark:text-white">{{ formatNumber(turbine.tower_vibration_fa, 2) }}</p>
            <p class="text-xs text-slate-500">Fore-Aft</p>
          </div>
          <div class="bg-slate-50 dark:bg-slate-900 p-2 rounded-lg text-center">
            <p class="text-lg font-bold text-slate-900 dark:text-white">{{ formatNumber(turbine.tower_vibration_ss, 2) }}</p>
      ...
      <div class="flex flex-col md:flex-row items-center gap-8">
        ...
        <div class="flex-1 w-full">
          <div class="flex items-center justify-between mb-6">
            ...
            <span :class="['px-3 py-1 rounded-full text-xs font-bold', getStatusBadgeClass(turbine.blade_status?.color)]">
              {{ turbine.blade_status?.label }}
            </span>
          </div>

          <div class="space-y-3">
            <div v-for="i in 3" :key="i" class="flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-slate-900/50">
              <div class="flex items-center gap-3">
                 <span class="w-6 h-6 flex items-center justify-center rounded-full bg-slate-200 dark:bg-slate-700 text-xs font-bold">
                   {{ i }}
                 </span>
              ...
              <div class="flex items-center gap-4">
                ...
                <div :class="['w-2 h-2 rounded-full', getBladeStatusDot(turbine[`blade${i}_vibration`])]"></div>
              </div>
            </div>
          </div>
        </div>

        <div class="flex-shrink-0 p-4 bg-slate-50 dark:bg-slate-900 rounded-full border border-slate-100 dark:border-slate-800">
          <svg class="w-48 h-48 animate-[spin_10s_linear_infinite]" viewBox="-50 -50 100 100">
            <circle cx="0" cy="0" r="14" :fill="getLedColor(turbine.blade_status?.color)" class="transition-colors duration-500" />
            <circle cx="0" cy="0" r="14" fill="none" class="stroke-slate-900 dark:stroke-white" stroke-width="0.6" />
