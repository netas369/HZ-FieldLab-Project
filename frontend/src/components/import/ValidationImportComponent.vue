<template>
  <div class="transition-colors duration-200">
    <h2 class="text-xl font-bold mb-4 text-gray-900 dark:text-white">Step 4: Validate & Import Data</h2>
    ...
    <div class="space-y-6">
      <div v-if="!validationStarted" class="space-y-4">
        <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <p class="text-sm text-blue-800 dark:text-blue-200 mb-2"><strong>Total Rows:</strong> {{ csvData.rowCount }}</p>
              <p class="text-sm text-blue-800 dark:text-blue-200 mb-2"><strong>Turbines:</strong> {{ keyColumns.uniqueTurbines.length }}</p>
              <p class="text-sm text-blue-800 dark:text-blue-200"><strong>Timestamp Column:</strong> {{ keyColumns.timestampColumn }}</p>
            </div>
            <div>
              <p class="text-sm text-blue-800 dark:text-blue-200 font-semibold mb-2">Data Categories:</p>
              <ul class="space-y-1">
      ...
      <div v-if="validating" class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6 text-center">
        ...
        <p class="text-sm text-blue-700 dark:text-blue-300">Checking {{ csvData.rowCount }} rows for errors</p>
      </div>

      <div v-if="validationComplete && !importing">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div class="bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 rounded-lg p-4">
            <div class="text-sm text-gray-600 dark:text-slate-400">Total Rows</div>
            <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ validationResults.totalRows }}</div>
          </div>
          <div class="bg-green-50 dark:bg-green-900/20 border border-green-300 dark:border-green-800 rounded-lg p-4">
            <div class="text-sm text-green-700 dark:text-green-300">Valid Rows</div>
            <div class="text-2xl font-bold text-green-700 dark:text-green-400">{{ validationResults.validRows }}</div>
          </div>
          <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-300 dark:border-yellow-800 rounded-lg p-4">
            <div class="text-sm text-yellow-700 dark:text-yellow-300">Warnings</div>
            <div class="text-2xl font-bold text-yellow-700 dark:text-yellow-400">{{ validationResults.warningRows }}</div>
          </div>
          <div class="bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-800 rounded-lg p-4">
            ...
            <div class="text-2xl font-bold text-red-700 dark:text-red-400">{{ validationResults.invalidRows }}</div>
          </div>
        </div>
          ...
          <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 mb-4">
            <div class="flex items-start justify-between">
              <div class="flex items-start">
                ...
                <div>
                  <h4 class="font-semibold text-red-900 dark:text-red-100">Validation Errors ({{ validationResults.errors.length }})</h4>
                  <p class="text-sm text-red-800 dark:text-red-200 mt-1">These rows will be skipped</p>
                </div>
              </div>
              <button @click="showErrors = !showErrors" class="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 font-medium text-sm">
                {{ showErrors ? 'Hide' : 'Show' }} Details
              </button>
            </div>
          </div>

          <div v-if="showErrors" class="border border-red-300 dark:border-red-800 rounded-lg overflow-hidden mb-4">
            <div class="overflow-x-auto max-h-96">
              <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                <thead class="bg-red-50 dark:bg-red-900/40">
                <tr>
                  <th class="px-4 py-2 text-left text-xs font-medium text-red-900 dark:text-red-100 uppercase">Row</th>
                  <th class="px-4 py-2 text-left text-xs font-medium text-red-900 dark:text-red-100 uppercase">Error</th>
                </tr>
                </thead>
                <tbody class="bg-white dark:bg-slate-900 divide-y divide-gray-200 dark:divide-slate-800">
                <tr v-for="(error, index) in validationResults.errors.slice(0, 50)" :key="index">
                  <td class="px-4 py-2 text-sm text-gray-900 dark:text-slate-300">{{ error.row }}</td>
                  <td class="px-4 py-2 text-sm text-red-700 dark:text-red-300">{{ error.message }}</td>
                </tr>
                </tbody>
              </table>
            </div>
          ...
          <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4 mb-4">
            <div class="flex items-start justify-between">
              <div class="flex items-start">
                ...
                <div>
                  <h4 class="font-semibold text-yellow-900 dark:text-yellow-100">Warnings ({{ validationResults.warnings.length }})</h4>
                  <p class="text-sm text-yellow-800 dark:text-yellow-200 mt-1">These rows will be imported but have potential issues</p>
                </div>
              </div>
              <button @click="showWarnings = !showWarnings" class="text-yellow-600 dark:text-yellow-400 hover:text-yellow-800 dark:hover:text-yellow-300 font-medium text-sm">
                {{ showWarnings ? 'Hide' : 'Show' }} Details
              </button>
            </div>
          </div>

          <div v-if="showWarnings" class="border border-yellow-300 dark:border-yellow-800 rounded-lg overflow-hidden mb-4">
            <div class="overflow-x-auto max-h-96">
              <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                <thead class="bg-yellow-50 dark:bg-yellow-900/40">
                <tr>
                  <th class="px-4 py-2 text-left text-xs font-medium text-yellow-900 dark:text-yellow-100 uppercase">Row</th>
                  <th class="px-4 py-2 text-left text-xs font-medium text-yellow-900 dark:text-yellow-100 uppercase">Warning</th>
                </tr>
                </thead>
                <tbody class="bg-white dark:bg-slate-900 divide-y divide-gray-200 dark:divide-slate-800">
                <tr v-for="(warning, index) in validationResults.warnings.slice(0, 50)" :key="index">
                  <td class="px-4 py-2 text-sm text-gray-900 dark:text-slate-300">{{ warning.row }}</td>
                  <td class="px-4 py-2 text-sm text-yellow-700 dark:text-yellow-300">{{ warning.message }}</td>
                </tr>
                </tbody>
              </table>
            </div>
        ...
        <div v-if="validationResults.validRows > 0" class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4 mb-6">
          <div class="flex items-start">
        ...
        <div class="flex justify-between">
          ...
          <button v-if="validationResults.validRows > 0" @click="startImport"
                  class="px-6 py-2 bg-green-600 hover:bg-green-700 dark:bg-green-600 dark:hover:bg-green-500 text-white rounded-lg transition-colors shadow-sm">
      ...
      <div v-if="importing" class="space-y-4">
        <div v-if="totalChunks === 0" class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6 text-center">
          <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 dark:border-blue-400 mx-auto mb-4"></div>
          <h3 class="font-semibold text-blue-900 dark:text-blue-100 mb-2">Importing Data...</h3>
          <p class="text-sm text-blue-700 dark:text-blue-300">Processing {{ csvData.data.length.toLocaleString() }} rows</p>
          <div class="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-2 mt-4 max-w-md mx-auto">
            <div class="bg-blue-600 dark:bg-blue-500 h-2 rounded-full transition-all duration-300" :style="{ width: importProgress + '%' }"></div>
          </div>
          <p class="text-xs text-blue-600 dark:text-blue-300 mt-2">{{ importStatusMessage }}</p>
        </div>

        <div v-else class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6">
          <div class="flex items-start mb-4">
            <div class="animate-spin rounded-full h-10 w-10 border-b-2 border-blue-600 dark:border-blue-400 mr-4 flex-shrink-0"></div>
            <div class="flex-1">
          ...
          <div class="w-full bg-blue-200 dark:bg-blue-900/50 rounded-full h-3 mb-3">
            <div
        ...
        <span v-if="chunkedProgress > 10" class="text-xs text-white font-medium">
          ...
          <div class="bg-blue-100 dark:bg-blue-900/40 border border-blue-300 dark:border-blue-700 rounded-lg p-3 mt-4">
            <div class="flex items-start">
