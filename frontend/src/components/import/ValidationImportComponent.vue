    ...
    <div class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <p class="text-sm text-blue-800 dark:text-blue-200 mb-2">
                <strong>Total Rows:</strong> {{ csvData.rowCount }}
              </p>
              <p class="text-sm text-blue-800 dark:text-blue-200 mb-2">
                <strong>Turbines:</strong> {{ keyColumns.uniqueTurbines.length }}
              </p>
              <p class="text-sm text-blue-800 dark:text-blue-200">
                <strong>Timestamp Column:</strong> {{ keyColumns.timestampColumn }}
              </p>
            </div>
            <div>
              ...
              <ul class="space-y-1">
                <li
                  v-for="cat in sensorMapping.mappedCategories.filter((c) => c.mappedCount > 0)"
                  :key="cat.id"
      ...
      <div v-if="validationComplete && !importing">
        ...
        <div v-if="validationResults.errors.length > 0" class="mb-6">
          ...
          <div
            v-if="showErrors"
            class="border border-red-300 dark:border-red-800 rounded-lg overflow-hidden mb-4"
          >
            <div class="overflow-x-auto max-h-96">
              <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                <thead class="bg-red-50 dark:bg-red-900/40">
                  <tr>
                    <th
                      class="px-4 py-2 text-left text-xs font-medium text-red-900 dark:text-red-100 uppercase"
                    >
                      Row
                    </th>
                    <th
                ...
                >
                  <tr v-for="(error, index) in validationResults.errors.slice(0, 50)" :key="index">
                    ...
                    <td class="px-4 py-2 text-sm text-red-700 dark:text-red-300">
                      {{ error.message }}
                    </td>
                  </tr>
                </tbody>
        ...
        <div v-if="validationResults.warnings.length > 0" class="mb-6">
              ...
              <div class="flex items-start">
                ...
                <div>
                  ...
                  <p class="text-sm text-yellow-800 dark:text-yellow-200 mt-1">
                    These rows will be imported but have potential issues
                  </p>
                </div>
              </div>
              <button
                class="text-yellow-600 dark:text-yellow-400 hover:text-yellow-800 dark:hover:text-yellow-300 font-medium text-sm"
          ...
          <div
            v-if="showWarnings"
            class="border border-yellow-300 dark:border-yellow-800 rounded-lg overflow-hidden mb-4"
          >
            <div class="overflow-x-auto max-h-96">
              <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                <thead class="bg-yellow-50 dark:bg-yellow-900/40">
                  <tr>
                    <th
                      class="px-4 py-2 text-left text-xs font-medium text-yellow-900 dark:text-yellow-100 uppercase"
                    >
                      Row
                    </th>
                    <th
                ...
                >
                  <tr
                    v-for="(warning, index) in validationResults.warnings.slice(0, 50)"
                    :key="index"
                  >
                    <td class="px-4 py-2 text-sm text-gray-900 dark:text-slate-300">
                      {{ warning.row }}
                    </td>
                    <td class="px-4 py-2 text-sm text-yellow-700 dark:text-yellow-300">
                      {{ warning.message }}
                    </td>
                  </tr>
                </tbody>
      ...
      <div v-if="importing" class="space-y-4">
        ...
        >
          <div class="flex items-start mb-4">
            <div
              class="animate-spin rounded-full h-10 w-10 border-b-2 border-blue-600 dark:border-blue-400 mr-4 flex-shrink-0"
            />
            <div class="flex-1">
              ...
              <p class="text-xs text-blue-600 dark:text-blue-400">
                Total rows: {{ csvData.data.length.toLocaleString() }} | Processed:
                {{ Math.floor((chunkedProgress / 100) * csvData.data.length).toLocaleString() }}
              </p>
            </div>
          ...
          <div
            class="bg-blue-100 dark:bg-blue-900/40 border border-blue-300 dark:border-blue-700 rounded-lg p-3 mt-4"
          >
            <div class="flex items-start">
              <svg
                class="w-4 h-4 text-blue-600 dark:text-blue-400 mt-0.5 mr-2 flex-shrink-0"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path
              ...
              <p class="text-xs text-blue-800 dark:text-blue-200">
                Large file detected - using chunked processing to prevent timeouts. This may take a
                few minutes. Please don't close this page.
              </p>
            </div>
          </div>
        </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const {
      ...
      currentChunk,
      totalChunks,
      startImport: startChunkedImport,
    } = useChunkedImport()
    ...
    const validateData = () => {
      ...
      props.csvData.data.forEach((row, index) => {
        ...
        const hasSensorData = Object.entries(props.sensorMapping.sensorMapping).some(
          ([, column]) => column && row[column] != null && row[column] !== ''
        )
