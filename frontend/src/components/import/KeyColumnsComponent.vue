  ...
  <div class="transition-colors duration-200">
    ...
    <div class="space-y-6">
        ...
        <div class="flex items-start">
          ...
          <div>
            ...
            <p class="text-sm text-blue-800 dark:text-blue-200 mt-2">
              We need to identify which columns contain the Turbine ID and Timestamp. The system
              will auto-detect these, but you can adjust if needed.
            </p>
          </div>
        </div>
      </div>
      ...
      >
        <div class="flex items-start">
          <svg
            class="w-5 h-5 text-green-600 dark:text-emerald-400 mt-0.5 mr-3"
            fill="currentColor"
            viewBox="0 0 20 20"
          >
            <path
          ...
          <div>
            <h4 class="font-semibold text-green-900 dark:text-emerald-100">
        ...
        <div class="space-y-4">
          <div>
            ...
            >
              ...
              <div
                v-if="selectedTurbineColumn"
                class="mt-3 p-3 bg-gray-50 dark:bg-slate-800 rounded border border-gray-200 dark:border-slate-700"
              >
                <p class="text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
                  Detected Turbine IDs ({{ uniqueTurbines.length }}):
                </p>
                <div class="flex flex-wrap gap-2">
                  <span
                    v-for="turbine in uniqueTurbines.slice(0, 20)"
                    :key="turbine"
                  ...
                  <span
                    v-if="uniqueTurbines.length > 20"
                    class="px-2 py-1 bg-gray-200 dark:bg-slate-700 text-gray-600 dark:text-slate-300 rounded text-sm"
                  >
          ...
          <div>
            ...
            >
              ...
              <p class="text-xs text-gray-500 dark:text-slate-500 mt-2">
                All rows will be imported for this turbine
              </p>
            </div>
          </div>
        </div>
      ...
      <div class="border border-gray-300 dark:border-slate-600 rounded-lg p-6 transition-colors">
        ...
        <select
          v-model="selectedTimestampColumn"
          class="w-full max-w-md px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 bg-white dark:bg-slate-700 text-gray-900 dark:text-white transition-colors"
          :class="
        ...
        >
          ...
          <div class="space-y-1">
            <p
              v-for="(sample, idx) in timestampSamples"
              :key="idx"
              class="text-sm font-mono text-gray-600 dark:text-slate-400"
            >
              {{ sample }}
            </p>
          </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const isValid = computed(() => {
      return (
        selectedTimestampColumn.value &&
        (turbineIdMode.value === 'column'
          ? selectedTurbineColumn.value
          : selectedSingleTurbine.value)
      )
    })
    ...
    const validate = () => {
      ...
      if (turbineIdMode.value === 'column' && !selectedTurbineColumn.value) {
        validationErrors.value.push(
          'Turbine ID column is required when CSV contains multiple turbines'
        )
      }
    ...
    const proceedToMapping = () => {
      ...
      emit('key-columns-identified', {
        ...
        uniqueTurbines:
          turbineIdMode.value === 'column' ? uniqueTurbines.value : [selectedSingleTurbine.value],
      })
    }
