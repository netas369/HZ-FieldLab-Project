...
const filteredTurbines = computed(() => {
  ...
  if (filters.searchQuery) {
    ...
    result = result.filter(
      (t) => t.id.toLowerCase().includes(query) || t.location.toLowerCase().includes(query)
    )
  }
...
const hasActiveFilters = computed(
  () => filters.searchQuery || filters.statusFilters.length > 0 || filters.showOnlyWithAlarms
)
...
// Stats calculations
const runningCount = computed(() => props.turbines.filter((t) => t.status === 'running').length)
