<template>
  <div class="space-y-2">
    ...
    >
      ...
      <div class="hidden sm:flex flex-col items-end mr-4 min-w-[80px] border-r border-slate-200 dark:border-slate-700 pr-6">
        <div v-if="turbine.healthData" class="flex flex-col items-end">
            <span class="text-2xl font-bold" :class="getHealthTextColor(turbine.healthData.overall_health.overall_health_score)">
...
const getStatusLabel = (status) => ({ running: 'Running', idle: 'Idle', maintenance: 'Maintenance', stopped: 'Stopped', error: 'Error' }[status] || 'Unknown')
