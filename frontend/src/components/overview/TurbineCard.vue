<template>
  ...
  >
    ...
    <div class="relative z-10">
      <div class="flex items-start justify-between mb-4">
        <div class="flex-1">
          <div class="flex items-center justify-between mr-4">
            <div>
              <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-1">{{ turbine.id }}</h3>
            ...
            <div v-if="turbine.healthData" class="flex flex-col items-end">
              <div class="flex items-center gap-1 bg-white/50 dark:bg-slate-800/50 px-2 py-1 rounded-lg border border-slate-100 dark:border-slate-700 backdrop-blur-sm">
                   <span class="text-lg font-bold" :class="getHealthTextColor(turbine.healthData.overall_health.overall_health_score)">
                      {{ turbine.healthData.overall_health.overall_health_score.toFixed(0) }}%
                   </span>
                <span v-if="turbine.healthData.overall_health.critical_components.length > 0" class="flex h-2 w-2 relative" title="Critical Issue">
        ...
        <div class="relative w-16 h-16 flex-shrink-0">
          ...
          <div :class="['absolute -top-1 -right-1 w-5 h-5 rounded-full border-2 border-white dark:border-slate-900', statusBadgeColor]">
      ...
      <div class="mb-4">
        <span :class="['inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full', statusClass]">
          <span :class="['w-2 h-2 rounded-full', statusBadgeColor]"></span>
          {{ statusLabel }}
        </span>
      </div>
    ...
    <div class="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
      <div :class="['absolute inset-0 rounded-xl', glowClass]"></div>
    </div>
  </div>
</template>
...
const statusBadgeColor = computed(() => {
  const colors = { running: 'bg-green-500', idle: 'bg-blue-500', maintenance: 'bg-amber-500', stopped: 'bg-red-500', error: 'bg-red-500' }
...
const statusLabel = computed(() => ({ running: 'Running', idle: 'Idle', maintenance: 'Maintenance', stopped: 'Stopped', error: 'Error' }[props.turbine.status] || 'Unknown'))

const borderClass = computed(() => {
  if (props.turbine.alarmSummary?.total > 0) return 'border-2 border-red-500 shadow-red-500/20'
  if (props.turbine.status === 'error' || props.turbine.status === 'stopped') return 'border-2 border-red-500'
...
const glowClass = computed(() => {
  const classes = { running: 'bg-gradient-to-t from-green-500/20 to-transparent', idle: 'bg-gradient-to-t from-blue-500/20 to-transparent', maintenance: 'bg-gradient-to-t from-amber-500/20 to-transparent', stopped: 'bg-gradient-to-t from-red-500/20 to-transparent', error: 'bg-gradient-to-t from-red-500/20 to-transparent' }
