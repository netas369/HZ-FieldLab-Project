...
const statusLabel = computed(() => {
  ...
  return labels[props.turbine.status] || 'Unknown'
})

const borderClass = computed(() => {
  if (props.turbine.alarmSummary?.total > 0) {
  ...
  if (props.turbine.status === 'maintenance') {
    return 'border-2 border-amber-400'
  }
  if (props.turbine.status === 'idle') {
    return 'border-2 border-blue-300'
  }
  if (props.turbine.status === 'running') {
