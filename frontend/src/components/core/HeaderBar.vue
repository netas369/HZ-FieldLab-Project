    ...
    <div class="max-w-[1600px] mx-auto px-6 py-4">
      <div class="flex items-center justify-between">
          ...
          <div class="relative group">
            ...
            <div class="relative bg-gradient-to-br from-indigo-500 to-purple-600 p-2.5 rounded-xl shadow-lg">
              <svg viewBox="0 0 100 100" class="w-8 h-8 text-white animate-spin-slow" aria-hidden="true">
                ...
                <path d="M50 10 L50 40 L35 25 M50 40 L65 25" stroke="currentColor" stroke-width="3.5" fill="none"
                  stroke-linecap="round" />
                <path d="M50 10 L50 40 L35 25 M50 40 L65 25" stroke="currentColor" stroke-width="3.5" fill="none"
                  stroke-linecap="round" transform="rotate(120 50 50)" />
                <path d="M50 10 L50 40 L35 25 M50 40 L65 25" stroke="currentColor" stroke-width="3.5" fill="none"
                  stroke-linecap="round" transform="rotate(240 50 50)" />
              </svg>
            </div>
          </div>
        ...
        <div class="hidden lg:flex items-center gap-6">
          <div
            class="flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700">
            <div class="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
            <span class="text-sm font-medium text-slate-700 dark:text-slate-300">All Systems Online</span>
          </div>

          <div v-if="activeAlarmsCount > 0"
            class="flex items-center gap-2 px-4 py-2 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
            <svg class="w-4 h-4 text-red-600 dark:text-red-400" fill="currentColor" viewBox="0 0 20 20">
              <path fill-rule="evenodd"
                d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
                clip-rule="evenodd" />
            </svg>
            <span class="text-sm font-semibold text-red-700 dark:text-red-400">{{ activeAlarmsCount }} Active Alarm{{
              activeAlarmsCount > 1 ? 's' : '' }}</span>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="relative">
            <button @click="showUserMenu = !showUserMenu"
              ...
              <!-- Chevron -->
              <svg class="w-4 h-4 text-slate-400 transition-transform" :class="{ 'rotate-180': showUserMenu }"
            ...
            <transition name="dropdown">
              <div v-if="showUserMenu" v-click-outside="() => showUserMenu = false"
                ...
                <button v-for="item in userMenuItems" :key="item.label" @click="handleMenuAction(item.action)"
