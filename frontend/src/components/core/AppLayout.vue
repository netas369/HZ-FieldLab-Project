<template>
  ...
  >
    ...
    <Teleport to="body">
      ...
      >
        <div
          class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-auto"
        >
          <div class="p-6">
              ...
              <div>
                ...
                <span
                  :class="[
                    ...
                    getPriorityClass(state.selectedAlarm.priority),
                  ]"
                >
            ...
            <div class="space-y-4">
              <div>
                ...
                <p class="text-slate-600 dark:text-slate-400">
                  {{ state.selectedAlarm.description }}
                </p>
              </div>

              <div class="grid grid-cols-2 gap-4">
                <div>
                  <h4 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">
                    Turbine
                  </h4>
                  <p class="text-slate-900 dark:text-white font-medium">
                    {{ state.selectedAlarm.turbine }}
                  </p>
                </div>
                <div>
                  <h4 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">
              ...
              <div class="flex flex-wrap gap-3 pt-4">
                <button
                  v-if="
                    !state.selectedAlarm.acknowledged && state.selectedAlarm.status !== 'resolved'
                  "
      ...
      >
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-2xl w-full">
          <div class="p-6">
            ...
            <form class="space-y-4" @submit.prevent="handleMaintenanceSubmit">
              <div>
      ...
      >
        ...
        >
          ...
          <form class="p-6 space-y-4" @submit.prevent="handleSubmitMaintenanceFromAlarm">
            <!-- Linked Alarm Notice -->
            <div
              v-if="newMaintenanceTask.alarm_id"
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                >
                <select
                  v-model="newMaintenanceTask.turbine_id"
                  required
                ...
                >
                  ...
                  <option v-for="t in turbineStore.turbines" :key="t._api_id" :value="t._api_id">
                    {{ t.id }}
                  </option>
                </select>
              </div>
              <div>
                ...
                >
                <select
                  v-model="newMaintenanceTask.assigned_to"
                  class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
                >
            ...
            <div>
              ...
              >
              <input
                v-model="newMaintenanceTask.title"
                required
                type="text"
                class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
            ...
            <div>
              ...
              >
              <textarea
                v-model="newMaintenanceTask.description"
                rows="3"
                class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                >
                <select
                  v-model="newMaintenanceTask.priority"
                  class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
                >
              ...
              <div>
                ...
                >
                <select
                  v-model="newMaintenanceTask.status"
                  class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
                >
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                >
                <input
                  v-model="newMaintenanceTask.scheduled_date"
                  type="datetime-local"
                  class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
                />
              </div>
              <div>
                ...
                >
                <input
                  v-model="newMaintenanceTask.due_date"
                  type="datetime-local"
                  class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
                />
              </div>
            ...
            <div>
              ...
              >
              <input
                v-model.number="newMaintenanceTask.estimated_duration_minutes"
                type="number"
                min="1"
                class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
                placeholder="e.g., 120"
