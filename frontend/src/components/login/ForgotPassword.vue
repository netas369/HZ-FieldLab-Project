            ...
            <div v-if="resetToken" class="space-y-4">
                ...
                <div class="bg-gray-50 border border-gray-300 rounded-md p-4">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <button @click="copyToken" class="text-xs text-indigo-600 hover:text-indigo-700 font-medium">
                            {{ copied ? '✓ Copied' : 'Copy' }}
                        </button>
                    </div>
...
const handleSubmit = async () => {
    ...
    try {
        const response = await fetch(`${import.meta.env.VITE_API_BASE_URL?.replace('/api', '') || 'http://localhost:8000'}/user/forgot-password`, {
            method: 'POST',
            headers: {
