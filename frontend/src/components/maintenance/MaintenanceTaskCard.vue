<template>
  ...
  >
    <div class="flex items-start justify-between mb-3">
      <div class="flex-1">
        <div class="flex items-center gap-2 mb-1 flex-wrap">
          ...
          <span v-if="isOverdue" class="flex items-center gap-1 text-xs font-semibold text-red-600 dark:text-red-400">
        ...
        <h4 class="text-base font-semibold text-slate-900 dark:text-white mb-1">{{ task.title }}</h4>
        <p v-if="task.description" class="text-sm text-slate-600 dark:text-slate-400">{{ task.description }}</p>
      </div>
    </div>
    ...
    <!-- Completed info -->
    <div v-if="task.status === 'completed' && task.completed_at" class="mt-3 pt-3 border-t border-slate-200 dark:border-slate-700">
