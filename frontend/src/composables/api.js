...
const alarmStore = reactive({
  ...
  activeAlarms: computed(() =>
    alarmStore.alarms.filter((a) => !a.acknowledged && a.status !== 'resolved')
  ),
  criticalCount: computed(
    () => alarmStore.alarms.filter((a) => a.priority === 'Critical' && !a.acknowledged).length
  ),
...
const historyStore = reactive({
  ...
  removeFromHistory: (turbineId, entryId) => {
    if (historyStore.recentFetches[turbineId]) {
      historyStore.recentFetches[turbineId] = historyStore.recentFetches[turbineId].filter(
        (e) => e.id !== entryId
      )
      localStorage.setItem('scada_history_v1', JSON.stringify(historyStore.recentFetches))
...
async function fetchDashboard() {
  ...
  try {
    ...
    for (const apiTurbine of response.data) {
      ...
      turbineData.metrics.power_mw = turbineData.scadaData?.power_kw
        ? turbineData.scadaData.power_kw / 1000
        : 0
      turbineData.metrics.wind_ms = turbineData.scadaData?.wind_speed_ms
        ? parseFloat(turbineData.scadaData.wind_speed_ms)
        : 0
      turbineData.metrics.wind_speed_ms = turbineData.scadaData?.wind_speed_ms
        ? parseFloat(turbineData.scadaData.wind_speed_ms)
        : 0
      turbineData.metrics.rotor_rpm = turbineData.scadaData?.rotor_speed_rpm
        ? parseFloat(turbineData.scadaData.rotor_speed_rpm)
        : 0
      turbineData.metrics.generator_rpm = turbineData.scadaData?.generator_speed_rpm
        ? parseFloat(turbineData.scadaData.generator_speed_rpm)
        : 0
      turbineData.metrics.pitch_deg = turbineData.scadaData?.pitch_angle_deg
        ? parseFloat(turbineData.scadaData.pitch_angle_deg)
        : 0
      turbineData.metrics.ambient_temp_c = turbineData.scadaData?.ambient_temp_c
        ? parseFloat(turbineData.scadaData.ambient_temp_c)
        : 0
